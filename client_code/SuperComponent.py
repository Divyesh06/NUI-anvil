...
events_map={'hover':'mouseenter','hover_out':'mouseleave','click':'click',_F:_F,'input':'input',_E:_E,'lost_focus':'blur'}
...
class SuperComponent:
	...
	def add_to_html_structure(A,child,**C):
		B=child
		if not hasattr(B,'is_nui'):
			if in_designer:
				if not B.parent:A.form._add_component(B,**C)
			else:A.form._add_component(B,**C)
		else:
			if in_designer:
				if not B.parent:A.form._add_component(B,**C)
			else:A.form._add_component(B,**C)
			E=A.form.get_components().index(B);D=B.dom
			if D:
				for C in A.dom.querySelectorAll('[anvil-name="container-slot"]'):
					if C.contains(D):C.remove()
				try:A.dom.insertBefore(D,A.dom.children[E])
				except LookupError:A.dom.appendChild(D)
				for F in B.stylesheets:A.dom.appendChild(F)
	@property
	...
	def margin(self,value):
		C='margin';B=self;A=value;B._margin=A
		if not A:return
		...
		try:update_component_properties(B.form,{C:A})
		except ExternalError:pass
		B.set_property(C,A)
	@property
	def padding(self):return self._padding
	@padding.setter
	def padding(self,value):
		C='padding';B=self;A=value;B._padding=A
		if not A:return
		if isinstance(A,list):A=_B.join([px_convert.convert_to_px(str(A if A else 0))for A in A])
		else:A=_B.join([px_convert.convert_to_px(A)for A in A.split()])if isinstance(A,str)else px_convert.convert_to_px(A)
		try:update_component_properties(B.form,{C:A})
	...
	@focus_css.setter
	def focus_css(self,value):B=value;A=self;A._focus_css=B;A.states_css[_E]=B;A._update_other_stylesheet()
	@property
	...
	def children_css(self,value):
		...
		if B:
			if not A.children_stylesheet:A.children_stylesheet=document.createElement(_D);get_dom_node(A.form).appendChild(A.children_stylesheet);A.stylesheets.append(A.children_stylesheet)
	...
	def icon_css(self,value):
		...
		if B:
			if not A.icon_stylesheet:A.icon_stylesheet=document.createElement(_D);get_dom_node(A.form).appendChild(A.icon_stylesheet);A.stylesheets.append(A.icon_stylesheet)
	...
	def _update_stylesheet(A):
		...
		if B:
			if not A.stylesheet:A.stylesheet=document.createElement(_D);get_dom_node(A.form).appendChild(A.stylesheet);A.stylesheets.append(A.stylesheet)
	...
	def _update_other_stylesheet(A):
		...
		for(D,C)in A.states_css.items():
			...
			B+=f"\n:{D}\n{C}"
		if B.strip():
			if not A.other_stylesheet:A.other_stylesheet=document.createElement(_D);get_dom_node(A.form).appendChild(A.other_stylesheet);A.stylesheets.append(A.other_stylesheet)
